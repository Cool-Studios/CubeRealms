...
public class Tree extends Entity{
	...
	public boolean isCollidingTree(float x, float y)
	{
		if(Math.abs(GameScreen.camera.position.x-x)<3000)
		{
			if(isEclosed)
			{
                ...
                return isClicked(Stroncs[0], x, y + Stroncs[0].getHeight() / 2, 6, 0);
			}
			else
			{
                return isClicked(Spousse, x, y, 2, 16);
			}
		}
		return false;
	}
	...
	public boolean isBreakingTree(float x, float y)
	{
		if(Math.abs(GameScreen.camera.position.x-x)<3000)
		{
            ...
            return isClicked(Stroncs[1], x, y, 0, 0);
		}
		return false;
	}
	...
	{
        return (x > spr.getX() - margX && x < spr.getX() + spr.getWidth() + margX) && (y > spr.getY() - margY && y < spr.getY() + spr.getHeight() + margY / 3);
	}
