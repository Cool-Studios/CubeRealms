...
public class LightManager {
	...
	public LightManager() 
	{
		...
		for(int i = 0; i<9; i++)activeChunks.add(new Vec2(i%3, i/3));
	...
	{	
		...
		currentChunkX = Tools.getBounded((minx+(cw/2))/chunksize,chunkNumberX);
		currentChunkY = Math.min(chunkNumberY-2, Math.max(1, (miny+(ch/2))/chunksize));
		
		for(int i = 0; i<activeChunks.size(); i++)
		{
			int difx = Math.abs(activeChunks.get(i).x-currentChunkX);
			int dify =  Math.abs(activeChunks.get(i).y-currentChunkY);
			if((difx>1 && difx< Map.instance.width/chunksize -1) || (dify>1))
			{
				int ti = -404;
				int tj = -404;
					...
					for(int j=0;j<ch;j++)
					{
						...
						if(cell.lightFull)
						{
							if(Parameters.i.RGB)
							{
								...
								setLightIntern(i,j,Math.max(coll+0.1f, getLightChange(i,j,n)),n);
							}
							else
							{
