...
public class Map {
	...
	public Map(String fileName, int width, int height) 
	{
		...
		chunkNumberWidth = width/chunkWidth;
		chunkNumberHeight = height/chunkHeight;
	...
	public void DrawBack(SpriteBatch batch)
	{
		...
		for(int i = minX; i<maxX; i++)
		{
			...
			{
				if((!Tools.isFull(mainLayer.getBloc(i, j),mainLayer.getState(i, j)) || mainLayer.getBloc(i, j).transparent) && (!Tools.isFull(backLayer.getBloc(i, j),backLayer.getState(i, j)) || backLayer.getBloc(i, j).transparent))
				{
			...
			if(maxY == limit)
			{
				if((!Tools.isFull(mainLayer.getBloc(i, limit),mainLayer.getState(i, limit)) || mainLayer.getBloc(i, limit).transparent) && (!Tools.isFull(backLayer.getBloc(i, limit),backLayer.getState(i, limit)) || backLayer.getBloc(i, limit).transparent))
				{
