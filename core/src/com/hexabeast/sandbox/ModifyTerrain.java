...
public class ModifyTerrain {
	...
	public boolean treeCollision(int decalaX, int decalaY)
	{
		...
		{
            return !getCell(Map.instance.mainLayer, decalaX, decalaY - 1).fertile || !getCell(Map.instance.mainLayer, decalaX + 1, decalaY - 1).fertile;
		}
		return true;
	}
	...
	public boolean justone(MapLayer layer, int x, int y, int id)
	{
		...
		if(layer.getBloc(x-1, y).collide)return true;
        return layer.getBloc(x, y - 1).collide;
    }
	...
	{
        return (x > spr.getX() - margX && x < spr.getX() + spr.getWidth() + margX) && (y > spr.getY() && y < spr.getY() + spr.getHeight());
	}
}
