...
public class MapLayer {
	...
	public void Animate(int i, int j)
	{
		...
		{	
			setBloc(i, j, curBloc, (byte) (getState(i,j) +1), false);
				...
				if(tooDif)
				{
					for(int l = 0; l<2; l++)
						for(int m = 0; m<2; m++)
						{
							...
							int srcX = TextureManager.instance.CoordinatesBlocs[curBloc.Id].x+ (getState(i,j)%10)%3 *10+4*l;
							int srcY = TextureManager.instance.CoordinatesBlocs[curBloc.Id].y+ (getState(i,j)%10)/3 *10+4*m;
							int srcWidth = 4;
							int srcHeight = 4;
					...
					if(tooDif)
					{
						for(int l = 0; l<2; l++)
							for(int m = 0; m<2; m++)
							{
								...
								int srcX = TextureManager.instance.CoordinatesBlocs[curBloc.Id].x+ getState(i,j)%9 *10+4*l;
								int srcY = TextureManager.instance.CoordinatesBlocs[curBloc.Id].y+ getState(i,j)/9 *10+4*m;
								int srcWidth = 4;
								int srcHeight =  4;
